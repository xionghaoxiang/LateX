...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    Time overall & 47.53min & 48.58min & 46.48min & 48.20min \\
...
Combining the above equations, and then by \( m \cdot P_{out} = F_{\text{total}} \cdot v \), we can obtain the conversion relationship between \( P_{out} \) and \( v \), as shown in the right of fig. 5.
...
With the help of \textit{Mathematica}, we obtain the curve of $P_{out}(x)$, and the result is shown in fig.\ref{fig:image15}. Compared with 2021 UCI, the Tokyo circuit is more complicated; the difficulty of the riders' power-distribution strategy has also increased significantly.
\vspace{-0.5cm}
\begin{figure}[H]
  \centering
...
\begin{figure}[ht]
  ...
  \end{minipage}
\end{figure}
fig. \ref{fig:image19} shows the changes of and curves (moving from bottom to top) when the wind speed
...
We use \textit{Python} to perturb the model within 10\% of
 the deviation from the wind speed and direction.
  The results are shown in fig. \ref{fig:image20}. where $\gamma$ is the local wind angle.
...
We conduct three experiments and get three
 ...
 Plot them in the same coordinate system as the ideal
  $P_{out}(x)$ curve, as shown in the left of fig. \ref{fig:image21}.
\begin{figure}[ht]
    \centering
    ...
    \end{minipage}
\caption{The results of the model. Left: The curve of $P_{out}(x)$ \& $P_{out}^{(\text{real})}(x)$. Middle: 1- of different strategies. Right: The Competition time of different strategies}
  \label{fig:image21}
\end{figure}
...
For the 4-person sprint cycling team, we designed the shield
 formation as shown in the left of fig. \ref{fig:image22}. The numbers 
