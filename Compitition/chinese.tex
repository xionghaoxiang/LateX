...
& D(t)=\begin{cases}
\|( \mathbf{P}_S\left( t \right) -\mathbf{P}_M\left( t \right))\| & \text{if } r \leq 0\\
\|\left( \mathbf{P}_T-\mathbf{P}_S\left( t \right) \right)\| & \text{if } r \geq 1 \\
\frac{\left( \mathbf{P}_S\left( t \right) -\mathbf{P}_M\left( t \right) \right) \times \left( \mathbf{P}_T-\mathbf{P}_S\left( t \right) \right)}{||\mathbf{P}_T\ -\ \mathbf{P}_M\left( t \right) ||^2} & \text{if } 0<r<1 
\end{cases} \\
...
\midrule
\multicolumn{7}{c}{\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}cccc}
...
\begin{table}[htbp]
\centering
\caption{问题\,4 三机协同立体封锁最终策略（坐标精简版）}
\label{tab:4}
